            ...
            for param_name, shard_name, shard_id in stacked_params_mapping:
                ...
                for (
                    ...
                    if moe_name not in params_dict:
                        # compressed-tensors NVFP4: "w2_weight.suffix"
                        # should be "w2_suffix" (e.g. w2_weight_packed)
                        moe_name = moe_name.replace(
                            "w2_weight.", "w2_"
                        ).replace(
                            "w13_weight.", "w13_"
                        )
                    ...
                    # Determine the correct weight_name suffix for
                    # FusedMoE's weight_loader. For bare Gemma4 names
                    # (no suffix), append ".weight". For compressed-
                    # tensors NVFP4 (has suffix like .weight_packed),
                    # extract the real suffix after the proj name.
                    suffix = name[name.find(weight_name) + len(weight_name):]
                    if suffix:
                        # e.g. ".weight_packed" → "down_proj.weight_packed"
                        wl_name = weight_name + suffix
                    else:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _weight_iterator():
            ...
            for name, weight in weights:
                ...
                # compressed-tensors NVFP4: per-expert tensors like
                # layers.X.experts.{id}.{proj}.{suffix} need .moe.
